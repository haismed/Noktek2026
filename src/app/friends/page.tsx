...
export default function FriendsPage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSearchResults(users.filter((u: any) => u.uid!== user?.uid));
    } catch (error) {
      console.error(error);
      toast({
        variant: "destructive",
  ...
  const handleAddFriend = async (target: UserItem) => {
    try {
  ...
  const handleUnfriend = async (friendId?: string) => {
    if (!friendId) return;
    const confirmed = window.confirm("هل أنت متأكد من حذف الصديق؟");
    if (!confirmed) return;
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 pt-6 pb-32">
      ...
      <Tabs defaultValue="find" className="w-full">
        <TabsList className="grid w-full grid-cols-4 bg-card h-14 rounded-2xl p-1 mb-8 border-border">
          <TabsTrigger value="find" className="rounded-xl font-bold">
            بحث
          </TabsTrigger>
          ...
          <TabsTrigger value="sent" className="rounded-xl font-bold">
            المرسلة
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="friends">
          <div className="space-y-4">
            {myFriends.map((friend) => (
              <Card key={friend.id} className="rounded-2xl">
                <CardContent className="p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <Avatar className="w-12 h-12">
