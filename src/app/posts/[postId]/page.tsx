...
import { ArrowRight, Send, MessageCircle, Clock, Sparkles, CheckCircle2, Lock, Zap, AlertCircle, UserCheck, Beaker, Loader2 } from "lucide-react";
...
import { COMMENT_REWARDS, ECONOMIC_MODEL } from "@/lib/platform-service";
...
export default function PostDetailsPage({ params }: { params: Promise<{ postId: string }> }) {
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setSeconds(prev => {
        ...
        // صرف المكافآت عند الوصول للثواني المطلوبة
  ...
  // حساب الثواني المتبقية لأقرب مكافأة
  let nextGoal = 45;
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {!isAuthor && !allRewardsDone && (
          <div className="flex items-center gap-2 bg-primary/10 px-3 py-1.5 rounded-full animate-in fade-in slide-in-from-top-2">
            ...
            <span className="text-[10px] font-black text-primary">مكافأة بعد: {remaining} ثانية</span>
          </div>
        )}
      </div>

      {/* تمرير بيانات التفاعل لـ PostCard لضمان التحديث الحي */}
      ...
      <div className="mt-8 space-y-6">
        ...
        <div className="space-y-4">
          {comments.map((comment) => (
            <div key={comment.id} className="bg-card p-4 rounded-2xl border border-border flex gap-3">
